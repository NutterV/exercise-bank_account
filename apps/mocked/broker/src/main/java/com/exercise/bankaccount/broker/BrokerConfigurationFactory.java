...
final class BrokerConfigurationFactory {
	/**
	 * Builds a non-persistent, unsecured broker configuration with both in-vm and
	 * TCP acceptors.
	 *
	 * @param properties
	 *            runtime properties that define broker endpoints
	 * @return configured Artemis server configuration
	 * @throws Exception
	 *             if Artemis rejects the supplied acceptor configuration
	 */
	Configuration create(BrokerRuntimeProperties properties) throws Exception {
		Configuration configuration = new ConfigurationImpl().setPersistenceEnabled(false).setSecurityEnabled(false);
