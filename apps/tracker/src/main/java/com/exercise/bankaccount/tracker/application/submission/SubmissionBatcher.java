...
/**
 * Queues sealed submission windows and batches them on a dedicated worker thread.
 */
@Component
public class SubmissionBatcher implements SubmissionProcessor {
	 ...
	 *
	 * @param trackerSubmissionProperties configured submission batching settings
	 * @param batchedSubmissionPublisher publisher for completed audit submissions
	 */
	public SubmissionBatcher(
		TrackerSubmissionProperties trackerSubmissionProperties,
		BatchedSubmissionPublisher batchedSubmissionPublisher
	) {
		this(
			trackerSubmissionProperties.maxBatchTotal(),
	...
	/**
	 * Test-friendly constructor that allows the worker executor and final consumer to be controlled explicitly.
	 *
	 * @param maxBatchTotal configured maximum absolute total allowed per batch
	 * @param auditSubmissionConsumer consumer invoked after a queued submission has been batched
	 * @param workerExecutor executor running the batching loop
	 */
	SubmissionBatcher(BigDecimal maxBatchTotal, Consumer<AuditSubmission> auditSubmissionConsumer, ExecutorService workerExecutor) {
	 ...
	 *
	 * @param transactions completed submission window ready for background batching
	 */
	@Override
	...
	AuditSubmission buildSubmission(List<Transaction> transactions) {
		...
		List<Transaction> sortedTransactions = transactions.stream()
			.sorted(Comparator.comparing(Transaction::absoluteAmount).reversed())
			.toList();
		...
		return new AuditSubmission(
			batches.stream()
				.map(batch -> new AuditBatch(batch.totalValue(), batch.transactionCount()))
				.toList()
		);
	}
	...
	private static void publishSubmission(BatchedSubmissionPublisher batchedSubmissionPublisher, AuditSubmission auditSubmission) {
		try {
